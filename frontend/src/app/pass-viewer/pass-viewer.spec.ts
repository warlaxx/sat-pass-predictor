...
const point = (seconds: number, azimuthDeg: number, elevationDeg: number): TrackPointDto => ({
  ...
  illuminated: false, subPoint: { latitudeDeg: 45, longitudeDeg: 4, altitudeKm: 420 },
});
...
describe('PassViewer', () => {
  it('renders the requested threshold and resets playback when the selection changes', async () => {
    ...
    expect(fixture.nativeElement.textContent).toContain('Illumination not yet calculated');
